...
async function tabTrains(main) {
  ...
  main.append(el("div", { class: "tiles" },
    ...
    tile("Errors", r.trains.filter((t) => t.errors.length).length, r.trains.filter((t) => t.errors.length).map((t) => t.name).slice(0, 3).join(", ") || "none", r.trains.some((t) => t.errors.length) ? "bad" : "ok"),
  ));
  if (!r.trains.length && !r.stations.length) { main.append(el("div", { class: "empty", text: "No trains or stations in this save." })); return; }

  // ---- trains
  if (state.sel.train == null || !r.trains.some((t) => t.name === state.sel.train)) state.sel.train = r.trains[0]?.name ?? null;
  const grid = el("div", { class: "grid" });
  main.append(grid);
  const trainsCard = el("div", { class: "card" }, el("h2", {}, "Trains", el("span", { class: "hint", text: "live · click a train for its timetable and history" })));
  const trainDetail = el("div", { class: "card" });
  const drawTrainDetail = () => {
    const t = r.trains.find((x) => x.name === state.sel.train);
    trainDetail.replaceChildren();
    if (!t) return;
    trainDetail.append(el("h2", {}, t.name, el("span", { class: "pill " + STATE_PILL[t.state], text: t.state })),
      el("div", { class: "muted small", text: `${t.status} · ${t.speed} km/h · ${t.locomotives} loco + ${t.cars - t.locomotives} cars · payload ${fmtPct(t.payloadPct)} (${fmtNum(t.payloadT)} of ${fmtNum(t.maxPayloadT)} t) · ${fmtNum(t.powerMW, 1)} MW · at ${t.location}` }));
    if (t.errors.length) trainDetail.append(el("ul", { class: "issues" }, t.errors.map((e) => el("li", { text: e }))));
    trainDetail.append(el("h3", { text: "Timetable" }),
      el("ol", { class: "timetable" }, t.timetable.map((stop, i) => el("li", { class: i === t.timetableIndex ? "cur" : null }, el("span", { text: stop }), i === t.timetableIndex ? el("span", { class: "pill ok", text: t.state === "docked" ? "here" : "next" }) : null))));
    trainDetail.append(el("h3", { text: "Cargo" }), t.cargo.length ? el("ul", { class: "plain" }, t.cargo.map((x) => el("li", { text: `${fmtNum(x.amount)} ${x.name}` }))) : el("p", { class: "muted small", text: "empty" }));
    trainDetail.append(el("h3", { text: `Dock history · last ${state.range}` }), visitsTable(visits.filter((v) => v.train === t.name), r.now, { hideTrain: true }));
  };
  const trainCols = [
    { key: "name", label: "Train" },
    { key: "state", label: "State", render: (t) => el("span", { class: "pill " + STATE_PILL[t.state], text: t.state }) },
    { key: "nextStop", label: t => t, render: (t) => t.state === "docked" ? `at ${t.station ?? "?"}` : `→ ${t.nextStop ?? t.station ?? "?"}` },
    { key: "speed", label: "km/h", num: true },
    { key: "payloadPct", label: "Payload", num: true, render: (t) => fmtPct(t.payloadPct) },
    { key: "cargo", label: "Cargo", sort: (t) => t.cargo.reduce((n, x) => n + x.amount, 0), render: (t) => cargoText(t.cargo) },
    { key: "errors", label: "Errors", sort: (t) => t.errors.length, render: (t) => (t.errors.length ? el("span", { class: "neg", text: t.errors.join("; ") }) : "") },
  ];
  trainCols[2].label = "Stop";
  trainsCard.append(table(r.trains, trainCols, { initialSort: { key: "name", dir: 1 }, selected: (t) => t.name === state.sel.train, onRow: (t) => { state.sel.train = t.name; renderTab(); } }));
  grid.append(trainsCard, trainDetail);
  drawTrainDetail();

  // ---- stations
  if (state.sel.station == null || !r.stations.some((s) => s.name === state.sel.station)) state.sel.station = r.stations[0]?.name ?? null;
  const grid2 = el("div", { class: "grid" });
  main.append(grid2);
  const stationsCard = el("div", { class: "card" }, el("h2", {}, "Stations", el("span", { class: "hint", text: "live · click a station for its platforms and history" })));
  const stationDetail = el("div", { class: "card" });
  const stationCols = [
    ...
    { key: "top", label: "Items", sort: (s) => s.topItems[0]?.amount ?? 0, render: (s) => cargoText(s.topItems) },
    { key: "docked", label: "Docked", render: (s) => s.docked ?? "" },
    { key: "inbound", label: "Inbound", sort: (s) => s.inbound.length, render: (s) => s.inbound.join(", ") },
    { key: "scheduled", label: "Scheduled", num: true, sort: (s) => s.scheduled.length, render: (s) => String(s.scheduled.length) },
  ];
  stationsCard.append(table(r.stations, stationCols, { initialSort: { key: "name", dir: 1 }, selected: (s) => s.name === state.sel.station, onRow: (s) => { state.sel.station = s.name; renderTab(); } }));
  grid2.append(stationsCard, stationDetail);
  const st = r.stations.find((x) => x.name === state.sel.station);
  if (st) {
    stationDetail.append(el("h2", {}, st.name, st.fuseTripped ? el("span", { class: "pill bad", text: "fuse tripped" }) : null),
      el("div", { class: "muted small", text: `${st.platforms.length} platforms · stock ${fmtNum(st.stock)} · transfer ${fmtNum(st.transferRate, 2)} · docked ${st.docked ?? "none"} · inbound ${st.inbound.join(", ") || "none"} · at ${st.location}` }));
    stationDetail.append(el("h3", { text: "Platforms" }), table(st.platforms, [
      ...
      { key: "inventory", label: "Items", sort: (p) => p.stock, render: (p) => cargoText(p.inventory) },
      { key: "transferRate", label: "Transfer", num: true, render: (p) => fmtNum(p.transferRate, 2) },
    ], { initialSort: { key: "index", dir: 1 } }));
    stationDetail.append(el("h3", { text: "Scheduled trains" }), st.scheduled.length ? el("ul", { class: "plain" }, st.scheduled.map((n) => el("li", {}, el("a", { href: "#", onclick: (e) => { e.preventDefault(); state.sel.train = n; renderTab(); } }, n)))) : el("p", { class: "muted small", text: "no train has this station on its timetable" }));
    stationDetail.append(el("h3", { text: `Dock history · last ${state.range}` }), visitsTable(visits.filter((v) => v.station === st.name), r.now, { hideStation: true }));
    const { card, host } = chartCard(`${st.name} · transfer rate per platform`);
    main.append(el("div", { class: "grid wide" }, card));
    try {
      ...
      lineChart(host, { x: pv.x, gaps: pv.gaps, epochs: pv.epochs, unit: "", height: 180, fmt: (v) => fmtNum(v, 2), series: [...pv.keys].map(([k, row]) => ({ label: `Platform ${Number(k) + 1}`, data: row.transfer_rate })) });
      card.append(el("div", { class: "chart-note", text: resNote(pv) }));
    } catch (e) { if (e instanceof LoginRequired) throw e; host.replaceChildren(el("div", { class: "chart-empty error", text: String(e.message) })); }
  }
}
